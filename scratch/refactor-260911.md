...
3. [Functions That Could Be Made Member Functions](#3-functions-that-could-be-made-member-functions)
   ...
   - [B. String Operations (`OTypeString` / `OTypeDynString` / `OTypeStrView`)](#b-string-operations-otypestring--otypedynstring--otypestrview)
   - [C. CString Operations (`OTypeCString`)](#c-cstring-operations-otypecstring)
   - [D. Compound & Struct Type Helpers (`OCompoundType` / `OTypeObject`)](#d-compound--struct-type-helpers-ocompoundtype--otypeobject)
   - [E. Property & Method Matching (`OValSymProperty` & `OValSymFunc`)](#e-property--method-matching-ovalsymproperty--ovalsymfunc)
   - [F. Function Types (`OTypeFunc`)](#f-function-types-otypefunc)
   - [G. Expression Helpers (`OPropertyExpr`, `OLValueExpr`, `OExpr`)](#g-expression-helpers-opropertyexpr-olvalueexpr-oexpr)
...
2. **Built-in Type Method Call Parsing Boilerplate**:
...
3. **Unknown Member Error Recovery in Postfix**:
...
4. **Polymorphic Method Registration**:
...
5. **C Varargs Default Argument Promotions**:
...
6. **Dynamic RTTI Traversal Loop Generation**:
...
7. **Pointer Arithmetic Type Validation**:
...
8. **Atomic Compilation Artifact Publishing**:
...
9. **Array LLVM IR Loop Generation**:
...
10. **Redundant Branch in `OCompoundType::ConvertFromExpr`**:
...
11. **Function Signature Matching**:
...
12. **Character Subtype Forwarding**:
...
### A. Dynamic Array Operations (`OTypeDynArray`)
Currently, [`compiler/types/otype_array.h:134-160`](file:///lindata2/workpr/dq-lang/compiler/types/otype_array.h#L134-L160) declares **27 free functions** taking `(OScope * scope, OTypeDynArray * dyntype, ...)`:
- `GenerateDynArrayDataPtr`, `GenerateDynArrayLength`, `GenerateDynArrayCapacity`, `GenerateDynArrayRefCount`, `GenerateDynArrayElementAddress`, `GenerateDynArraySlice`, `GenerateDynArrayCreate`, `GenerateDynArrayDestroy`, `GenerateDynArrayManagerValue`, `GenerateDynArrayAssignOther`, `GenerateDynArrayAssignData`, `GenerateDynArrayClear`, `GenerateDynArrayReserve`, `GenerateDynArrayCompact`, `GenerateDynArraySetLength`, `GenerateDynArraySetCapacity`, `GenerateDynArrayAppend`, `GenerateDynArrayAppendSlice`, `GenerateDynArrayPrepend`, `GenerateDynArrayPrependSlice`, `GenerateDynArrayInsert`, `GenerateDynArrayInsertSlice`, `GenerateDynArrayDelete`, `GenerateDynArrayClone`, `GenerateDynArrayPop`.
- **Recommendation:** Make them member methods on [`OTypeDynArray`](file:///lindata2/workpr/dq-lang/compiler/types/otype_array.h#L103):
  `dyntype->GenerateLength(scope, dynaddr)`, `dyntype->GenerateAppend(scope, dynaddr, value)`, etc.
- Also move [`DynArrayElementStorageType`](file:///lindata2/workpr/dq-lang/compiler/types/otype_array.cpp#L127) and [`DynArrayTypeInfo`](file:///lindata2/workpr/dq-lang/compiler/types/otype_array.cpp#L408) into `OTypeDynArray`.

### B. String Operations (`OTypeString` / `OTypeDynString` / `OTypeStrView`)
[`compiler/types/otype_string.h:92-120`](file:///lindata2/workpr/dq-lang/compiler/types/otype_string.h#L92-L120) declares **20+ free functions**:
- `GenerateStringLength`, `GenerateStringCapacity`, `GenerateStringRefCount`, `GenerateStringPChar` -> should be member methods on [`OTypeDynString`](file:///lindata2/workpr/dq-lang/compiler/types/otype_string.h#L21) / [`OTypeStrView`](file:///lindata2/workpr/dq-lang/compiler/types/otype_string.h#L41).
- `GenerateStringWcLen`, `GenerateStringWCharAt`, `GenerateStringWCharSlice`, `GenerateStringToWchars`, `GenerateStringGetChar`, `GenerateStringCharAddress`, `GenerateStringSetChar`, `GenerateStringSlice`, `GenerateStringEqual`, `GenerateStringConcat`, `GenerateStringCreate`, `GenerateStringIncRef`, `GenerateStringDestroy`, `GenerateStringAssignExpr`, `GenerateStringMethodCall`.
- In addition: [`IsTextSourceType(OType*)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_string.h#L92), [`IsStringComparableTextType(OType*)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_string.h#L93), and [`IsStringFamilyTextType(OType*)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_string.h#L94) belong directly on [`OType`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L309) (`type->IsTextSource()`, `type->IsStringComparable()`, `type->IsStringFamily()`).

### C. CString Operations (`OTypeCString`)
In [`compiler/types/otype_cstring.h:102-133`](file:///lindata2/workpr/dq-lang/compiler/types/otype_cstring.h#L102-L133), methods like `CanStoreFrom`, `GenerateStore`, and `GenerateDescriptor` are already member functions of `OTypeCString`. However, right alongside them are free functions:
- `GenerateCStringDataPtr(OScope*, OTypeCString*, LlValue*)`
- `GenerateCStringMetaField(OScope*, OTypeCString*, LlValue*, ECStringMetaField)`
- `GenerateCStringMethodCall(OScope*, OTypeCString*, LlValue*, ECStringMethod, ...)`
- `EmitSizedCStringCopy(...)`
- **Recommendation:** Make these member methods on [`OTypeCString`](file:///lindata2/workpr/dq-lang/compiler/types/otype_cstring.h#L24).

### D. Compound & Struct Type Helpers (`OCompoundType` / `OTypeObject`)
- [`AnalyzeStructLiteral(OCompoundType * type, OStructLit * literal, ...)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_compound.cpp#L1455-L1596) (140 lines in [`compiler/types/otype_compound.cpp`](file:///lindata2/workpr/dq-lang/compiler/types/otype_compound.cpp)) -> belongs on [`OCompoundType::AnalyzeLiteral(OStructLit * literal, ...)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_compound.h#L35).
- [`CollectStructInitFields(OCompoundType * type, ...)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_compound.cpp#L1348-L1377) -> belongs on [`OCompoundType::CollectInitFields(...)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_compound.h#L35).
- [`PropertyAccessorVisibleFrom(OTypeObject * owner, OCompoundType * decl_type, OValSym * accessor)`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser.cpp#L2178) -> belongs on [`OCompoundType::IsAccessorVisible(OValSym * accessor, OTypeObject * context_owner)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_compound.h#L35).

### E. Property & Method Matching (`OValSymProperty` & `OValSymFunc`)
- [`MatchPropertyMethod`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser.cpp#L2199) and [`ImportedPropertyMethodMatches`](file:///lindata2/workpr/dq-lang/compiler/ast/module_intf.cpp#L2837) -> should become member method [`OValSymProperty::MatchMethod(OValSymFunc * method, bool write)`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L650).
- [`FunctionSignature(OValSymFunc * afunc)`](file:///lindata2/workpr/dq-lang/compiler/ast/module_intf.cpp#L201-L256) (56 lines in [`compiler/ast/module_intf.cpp`](file:///lindata2/workpr/dq-lang/compiler/ast/module_intf.cpp)) -> belongs on [`OValSymFunc::SignatureText()`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L670).
- [`FunctionState(OValSymFunc * afunc)`](file:///lindata2/workpr/dq-lang/compiler/ast/module_intf.cpp#L258-L277) in [`compiler/ast/module_intf.cpp`](file:///lindata2/workpr/dq-lang/compiler/ast/module_intf.cpp) -> belongs on [`OValSymFunc::StateText()`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L670).
- [`FuncTypeOf(OValSymFunc * afunc)`](file:///lindata2/workpr/dq-lang/compiler/ast/module_intf.cpp#L190-L193) in [`compiler/ast/module_intf.cpp`](file:///lindata2/workpr/dq-lang/compiler/ast/module_intf.cpp) -> can simply be `afunc ? afunc->GetTypeFunc() : nullptr`.
- [`IsImplicitReceiverParam(OValSymFunc * afunc, OFuncParam * aparam, ...)`](file:///lindata2/workpr/dq-lang/compiler/ast/module_intf.cpp#L195-L199) -> belongs on [`OValSymFunc::IsImplicitReceiver(OFuncParam * param)`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L670) or `OFuncParam::IsImplicitReceiver()`.
- [`CloneMethodVisibleSignature(OValSymFunc * vsfunc)`](file:///lindata2/workpr/dq-lang/compiler/ast/expressions.cpp#L2675-L2692) in [`compiler/ast/expressions.cpp`](file:///lindata2/workpr/dq-lang/compiler/ast/expressions.cpp) -> belongs on [`OValSymFunc::CloneVisibleSignature()`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L670).
- [`ConstructorUserSignaturesMatch(OValSymFunc * left, OValSymFunc * right)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_compound.cpp#L245-L273) in [`compiler/types/otype_compound.cpp`](file:///lindata2/workpr/dq-lang/compiler/types/otype_compound.cpp) -> belongs on [`OValSymFunc::UserSignaturesMatch(OValSymFunc * other)`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L670).

### F. Function Types (`OTypeFunc`)
- [`FuncTypeName(OTypeFunc * sigtype)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_func.cpp#L1304-L1346) (43 lines in [`compiler/types/otype_func.cpp`](file:///lindata2/workpr/dq-lang/compiler/types/otype_func.cpp)) -> belongs on [`OTypeFunc::FormattedTypeName()`](file:///lindata2/workpr/dq-lang/compiler/types/otype_func.h#L45).
- [`FuncRefTypeName(OTypeFunc * sigtype, bool object_ref)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_func.cpp#L1348-L1356) -> belongs on [`OTypeFunc::FormattedRefTypeName(bool object_ref)`](file:///lindata2/workpr/dq-lang/compiler/types/otype_func.h#L45).
- [`CreateObjectFuncRefLlCallType(OTypeFunc * sigtype)`](file:///lindata2/workpr/dq-lang/compiler/ast/expressions.cpp#L2694-L2713) in [`compiler/ast/expressions.cpp`](file:///lindata2/workpr/dq-lang/compiler/ast/expressions.cpp) -> belongs on [`OTypeFunc::CreateObjectRefLlCallType()`](file:///lindata2/workpr/dq-lang/compiler/types/otype_func.h#L45).

### G. Expression Helpers (`OPropertyExpr`, `OLValueExpr`, `OExpr`)
- In [`compiler/ast/expressions.cpp:964-1026`](file:///lindata2/workpr/dq-lang/compiler/ast/expressions.cpp#L964-L1026), four static functions handle property code generation:
...
- [`ExceptionBaseType(OScope * scope)`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser_stmt.cpp#L30-L34) in [`compiler/parser/dqc_parser_stmt.cpp`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser_stmt.cpp) -> belongs on [`OScope::GetExceptionBaseType()`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L94).
- [`AddMethodUseRootScopes(OScope * dst_scope, OScope * root_scope)`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser_stmt.cpp#L73-L86) in [`compiler/parser/dqc_parser_stmt.cpp`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser_stmt.cpp) -> belongs on [`OScope::AddMethodUseRootScopes(OScope * root_scope)`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L94).
- [`EmitExpressionExceptionCheck(OScope * scope)`](file:///lindata2/workpr/dq-lang/compiler/ast/expressions.cpp#L50-L54) in [`compiler/ast/expressions.cpp`](file:///lindata2/workpr/dq-lang/compiler/ast/expressions.cpp) -> belongs on [`OScope::EmitExpressionExceptionCheck()`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L94).
- [`FindModuleUseByNamespace(const string & namespace_name)`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser_stmt.cpp#L57-L71) in [`compiler/parser/dqc_parser_stmt.cpp`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser_stmt.cpp) -> belongs on [`TDQModule::FindModuleUseByNamespace(...)`](file:///lindata2/workpr/dq-lang/compiler/ast/dq_module.h#L30).

### I. Type Interface Serialization (`OType` / `ODqmIfWriter`)
- [`WriteDqmIfTypeRef(ODqmIfWriter & writer, uint16_t arecid, OType * atype)`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.cpp#L39-L50) and [`WriteDqmIfTypeSpecInner(ODqmIfWriter & writer, OType * atype)`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.cpp#L222-L280) in [`compiler/ast/symbols.cpp`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.cpp) -> belongs on [`OType::WriteDqmIfTypeRef(...)`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L309) or [`ODqmIfWriter::WriteTypeRef(...)`](file:///lindata2/workpr/dq-lang/compiler/ast/dqm_if.h#L80).
- [`EffectiveStorageAlign(OType * atype, uint32_t aattr_align)`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.cpp#L562-L570) in [`compiler/ast/symbols.cpp`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.cpp) -> belongs on [`OType::EffectiveAlign(uint32_t attr_align)`](file:///lindata2/workpr/dq-lang/compiler/ast/symbols.h#L309).

---

## 4. Key Architectural Recommendations

1. **Encapsulate Built-in Operations onto Type Classes**:
   - Move the 27 free functions in `otype_array.h` to `OTypeDynArray` and the 20+ free functions in `otype_string.h` to `OTypeDynString` / `OTypeStrView`.
   - Polymorphize `GenerateAssignmentToAddress`: replace the manual `dynamic_cast` switch in [`compiler/ast/statements.cpp:62-92`](file:///lindata2/workpr/dq-lang/compiler/ast/statements.cpp#L62-L92) with a virtual `OType::GenerateAssignment(OScope * scope, LlValue * targetaddr, OExpr * value, bool volatile_store)`.

2. **Decompose Top-Heavy Parser Functions**:
   - Break [`ODqCompParserExpr::ParsePostfix`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser_expr.cpp#L2382) (583 lines) into distinct helper member methods: `ParsePostfixDotMember`, `ParsePostfixIndexOrSlice`, `ParsePostfixCall`, and `ParsePostfixPointerOps`.
   - Break [`ODqCompParserStmt::ReadStatementBlock`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser_stmt.cpp#L934) (325 lines) into an isolated `ParseStatement()` dispatcher, separating loop control and block boundary scanning from individual statement parsing.
   - Extract the nested lambdas in [`ODqCompParser::FinishFunctionDecl`](file:///lindata2/workpr/dq-lang/compiler/parser/dqc_parser.cpp#L1613) (481 lines) into private class member functions (`ReadFunctionBody`, `DeclareFunctionSymbol`, `ResolveForwardDecl`).

3. **Adopt RAII for `TRawCallArg`**:
...
4. **Deduplicate Shared Infrastructure**:
