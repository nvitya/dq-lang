...
class ODqProjectParser
{
public:
  ODqProjectFile & project;
  vector<string> default_package_paths;
  ...
  ODqProjectParser(ODqProjectFile & aproject,
                   const vector<string> & adefault_package_paths,
                   const vector<string> & acommand_line_package_paths)
  :
    project(aproject),
    default_package_paths(adefault_package_paths),
  ...
  pair<int, int> LineCol(const SProjectParseContext & ctx, const char * pos) const
  {
    int line = 1;
    int col = 1;
  ...
  bool Fail(const SProjectParseContext & ctx, const string & id, const string & message,
            const char * pos = nullptr)
  {
    if (project.diagnostics.empty())
    {
      ...
      project.diagnostics.push_back({ctx.filename, line, col, id, message});
    }
    return false;
  }

  bool SkipSpace(SProjectParseContext & ctx, bool include_line_end, bool & rsaw_line_end)
  {
    rsaw_line_end = false;
  ...
  bool SkipInlineSpace(SProjectParseContext & ctx)
  {
  ...
  bool RequireInlineSpaceResult(SProjectParseContext & ctx, const string & expected)
  {
    if (SkipInlineSpace(ctx)) return true;
    if (!project.diagnostics.empty()) return false;
    return Fail(ctx, "ProjectSyntax", expected + " before the end of the statement");
  }

  bool FinishStatement(SProjectParseContext & ctx)
  {
    bool saw_line_end = false;
    if (!SkipSpace(ctx, false, saw_line_end)) return false;
  ...
  bool ReadIdentifier(SProjectParseContext & ctx, string & rvalue, const string & what)
  {
    if (ctx.sp.ReadIdentifier(rvalue)) return true;
    return Fail(ctx, "ProjectSyntax", "Expected " + what);
  }

  bool ReadString(SProjectParseContext & ctx, string & rvalue)
  {
    if ((ctx.sp.readptr >= ctx.sp.bufend)
  ...
  bool ExpandVariables(SProjectParseContext & ctx, const string & value, string & rvalue)
  {
    rvalue.clear();
    size_t start = 0;
  ...
  bool ReadExpandedString(SProjectParseContext & ctx, string & rvalue)
  {
    string raw;
  ...
  bool ReadPath(SProjectParseContext & ctx, filesystem::path & rpath)
  {
    string value;
  ...
  bool ReadBool(SProjectParseContext & ctx, bool & rvalue)
  {
    string value;
  ...
  bool ReadInt(SProjectParseContext & ctx, int64_t & rvalue)
  {
    const char * start = ctx.sp.readptr;
    const char * p = start;
  ...
  bool CheckDuplicate(SProjectParseContext & ctx, const string & name)
  {
  ...
  {
    vector<string> result = default_package_paths;
    result.insert(result.end(), project.package_paths.begin(), project.package_paths.end());
    result.insert(result.end(), command_line_package_paths.begin(), command_line_package_paths.end());
    return result;
  }

  bool ReadPackagePathCall(SProjectParseContext & ctx, string & rvalue)
  {
    ...
    {
      return project.diagnostics.empty() ? Fail(ctx, "ProjectSyntax", "Expected '(' after PackagePath") : false;
    }
    ...
    if (!RequireInlineSpaceResult(ctx, "Expected ')'") || !ctx.sp.CheckSymbol(")"))
    {
      return project.diagnostics.empty() ? Fail(ctx, "ProjectSyntax", "Expected ')' after package name") : false;
    }
  ...
  bool ParseVariable(SProjectParseContext & ctx)
  {
    ...
    if (!RequireInlineSpaceResult(ctx, "Expected '='") || !ctx.sp.CheckSymbol("="))
    {
      return project.diagnostics.empty() ? Fail(ctx, "ProjectSyntax", "Expected '=' after project variable") : false;
    }
  ...
  bool ParseInclude(SProjectParseContext & ctx)
  {
  ...
  bool ParseDefine(SProjectParseContext & ctx)
  {
  ...
  bool ParseProperty(SProjectParseContext & ctx, const string & name)
  {
    ...
    if (!RequireInlineSpaceResult(ctx, "Expected '='") || !ctx.sp.CheckSymbol("="))
    {
      return project.diagnostics.empty() ? Fail(ctx, "ProjectSyntax", format("Expected '=' after \"{}\"", name)) : false;
    }
        ...
        || (name == "packagepath") || (name == "linkerpath") || (name == "linkobject"))
    {
      ...
      if      (name == "main")        project.main_file = path;
      else if (name == "output")      project.output_file = path;
      else if (name == "packagepath") project.package_paths.push_back(path.string());
      else if (name == "linkobject")  project.link_objects.push_back(path.string());
      else if (name == "linkerpath")  project.linker_args.push_back("--library-path=" + path.string());
      else                             project.linker_args.push_back("--script=" + path.string());
    }
    else if (name == "target")
    {
      string value;
    ...
    else if (name == "linkoption")
    {
      string value;
      if (!ReadExpandedString(ctx, value)) return false;
      project.linker_args.push_back(value);
    }
    else if ((name == "link") || (name == "debuginfo"))
    {
      bool value = false;
      if (!ReadBool(ctx, value)) return false;
      if (name == "link") project.link = value;
      else project.debuginfo = value;
    }
    else
    {
      int64_t value = 0;
  ...
  {
    project.filename = Canonical(top_file);
    project_dir = project.filename.parent_path();
    if (project.filename.extension() != ".dqproj")
    {
      SProjectParseContext ctx;
      ctx.filename = project.filename;
      ctx.text = " ";
      ctx.sp.Init(ctx.text.data(), ctx.text.size());
      return Fail(ctx, "ProjectExtension", "DQ project files must use the .dqproj extension");
    }
    if (!ParseFile(project.filename)) return false;
    if (project.main_file.empty())
    {
      SProjectParseContext ctx;
      ctx.filename = project.filename;
      ctx.text = " ";
...
{
  *this = ODqProjectFile();
  ODqProjectParser parser(*this, default_package_paths, command_line_package_paths);
  return parser.Load(top_file);
}
